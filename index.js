...
const createClient = (profile, userAgent, request = _request) => {
	...
	const journeysFromTrip = (fromTripId, previousStopover, to, opt = {}) => {
		...
		const depAtPrevStop = +new Date(previousStopover.departure || previousStopover.formerScheduledDeparture)
		...
		.then((d) => {
			...
			.map((journey, i) => {
				const fromLeg = journey.legs.find(l => l.id === fromTripId)
				if (!fromLeg) return journey

				try {
					const slicedFromLeg = sliceLeg(fromLeg, previousStopover.stop, fromLeg.destination)
					return Object.assign({}, journey, {
						legs: [
							...journey.legs.slice(0, i),
							slicedFromLeg,
							...journey.legs.slice(i + 2)
						]
					})
				} catch (err) {
					return journey
